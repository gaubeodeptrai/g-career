...
//トップページのカテゴリー4の初期設定
global $index_category4_options;
$index_category4_options[0] = array(
    'value' => 0,
    'label' => ''
);
$dp_cats4 = get_categories();
foreach( $dp_cats4 as $dp_cat4 ) :
 $index_category4_options[$dp_cat4->cat_ID] = array(
  'value' => $dp_cat4->cat_ID,
  'label' => $dp_cat4->cat_name
 );
endforeach;


//トップページのカテゴリー5の初期設定
global $index_category5_options;
$index_category5_options[0] = array(
    'value' => 0,
    'label' => ''
);
$dp_cats5 = get_categories();
foreach( $dp_cats5 as $dp_cat5 ) :
 $index_category5_options[$dp_cat5->cat_ID] = array(
  'value' => $dp_cat5->cat_ID,
  'label' => $dp_cat5->cat_name
 );
endforeach;


//トップページのカテゴリー6の初期設定
global $index_category6_options;
$index_category6_options[0] = array(
    'value' => 0,
    'label' => ''
);
$dp_cats6 = get_categories();
foreach( $dp_cats6 as $dp_cat6 ) :
 $index_category6_options[$dp_cat6->cat_ID] = array(
  'value' => $dp_cat6->cat_ID,
  'label' => $dp_cat6->cat_name
 );
endforeach;
...
function theme_options_do_page() {
 global $index_featured_options, $index_category1_options, $index_category2_options, $index_category3_options,$index_category4_options, $index_category5_options, $index_category6_options, $top_banner_type_options, $dp_upload_error;
...
<form method="post" action="options.php" enctype="multipart/form-data">
 ...
 <div id="tab-panel">
  ...
  <div id="tab-content3">
   ...
   <?php // 注目のカテゴリー記事4 ?>
   <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e('Featured category post4 setting', 'tcd-w');  ?></h3>
    <p><?php _e('Select category to use for featured category post4.<br />Leave this field blank if you don\'t want to use featured category post4.', 'tcd-w');  ?></p>
    <div class="theme_option_input">
     <select id="index_category4" name="dp_options[index_category4]">
      <?php
           foreach ( $index_category4_options as $option4 ) :
            $label = $option4['label'];
            $selected = '';
            if ( $options['index_category4'] == $option4['value']) {
             $selected = 'selected="selected"';
            } else {
             $selected = '';
            }
            echo '<option style="padding-right: 10px;" value="' . esc_attr( $option4['value'] ) . '" ' . $selected . '>' . $label . '</option>' ."\n";
           endforeach;
      ?>
    </select>
    </div>
   ...
   <?php // 注目のカテゴリー記事5 ?>
   <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e('Featured category post5 setting', 'tcd-w');  ?></h3>
    <p><?php _e('Select category to use for featured category post5.<br />Leave this field blank if you don\'t want to use featured category post5.', 'tcd-w');  ?></p>
    <div class="theme_option_input">
     <select id="index_category5" name="dp_options[index_category5]">
      <?php
           foreach ( $index_category5_options as $option5 ) :
            $label = $option5['label'];
            $selected = '';
            if ( $options['index_category5'] == $option5['value']) {
             $selected = 'selected="selected"';
            } else {
             $selected = '';
            }
            echo '<option style="padding-right: 10px;" value="' . esc_attr( $option5['value'] ) . '" ' . $selected . '>' . $label . '</option>' ."\n";
           endforeach;
      ?>
    </select>
    </div>
   ...
   <?php // 注目のカテゴリー記事6 ?>
   <div class="theme_option_field cf">
    <h3 class="theme_option_headline"><?php _e('Featured category post6 setting', 'tcd-w');  ?></h3>
    <p><?php _e('Select category to use for featured category post6.<br />Leave this field blank if you don\'t want to use featured category post6.', 'tcd-w');  ?></p>
    <div class="theme_option_input">
     <select id="index_category6" name="dp_options[index_category6]">
      <?php
           foreach ( $index_category6_options as $option6 ) :
            $label = $option6['label'];
            $selected = '';
            if ( $options['index_category6'] == $option6['value']) {
             $selected = 'selected="selected"';
            } else {
             $selected = '';
            }
            echo '<option style="padding-right: 10px;" value="' . esc_attr( $option6['value'] ) . '" ' . $selected . '>' . $label . '</option>' ."\n";
           endforeach;
      ?>
    </select>
    </div>
...
function theme_options_validate( $input ) {
 global $index_featured_options, $index_category1_options, $index_category2_options, $index_category3_options, $index_category4_options, $index_category5_options, $index_category6_options, $top_banner_type_options;
 ...
 // トップページのおすすめカテゴリー記事4の設定
 if ( ! isset( $input['index_category4'] ) )
  $input['index_category4'] = null;
 if ( ! array_key_exists( $input['index_category4'], $index_category4_options ) )
  $input['index_category4'] = null;

 // トップページのおすすめカテゴリー記事5の設定
 if ( ! isset( $input['index_category5'] ) )
  $input['index_category5'] = null;
 if ( ! array_key_exists( $input['index_category5'], $index_category5_options ) )
  $input['index_category5'] = null;

 // トップページのおすすめカテゴリー記事6の設定
 if ( ! isset( $input['index_category6'] ) )
  $input['index_category6'] = null;
 if ( ! array_key_exists( $input['index_category6'], $index_category6_options ) )
  $input['index_category6'] = null;
