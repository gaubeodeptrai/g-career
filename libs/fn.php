...
function srcEncode($txt){
    $txt = preg_replace('/\A[　\s]*(.*?)[　\s]*\z/u','$1',$txt);
...
function srcMix($txt){
    ...
    $txt = preg_split('/[\s]+/', $txt, -1, PREG_SPLIT_NO_EMPTY);
    return $txt;
}
...
function filter($val){
    ...
    is_array($val)
        ...
        //$val= array_filter($val); // 空を削除(配列の数を決めていないときに有効にすること）
    } else{
        $val = trim($val);
    }
   return $val;
...
function newFlg($str) {
    ...
    $date = date("Y-m-d",strtotime("-2week"));
...
function weekName($y="", $m="", $d=""){
    $y=empty($y)?date("Y"):$y;
    $m=empty($m)?date("m"):$m;
    $d=empty($d)?date("d"):$d;
    return weekLi(date("w", mktime(0, 0, 0, $m, $d, $y)));
}
...
function host($host="") {
    ...
    //ホストアドレス取得
    $hostadd = gethostbyaddr($ipadd);
    //ブラウザ
    $ua = $_SERVER["HTTP_USER_AGENT"];
    $host = $ipadd."/".$hostadd."/".$ua;
    return $host;
}
